...
private:
  ...
  int turn_double_move_;

  int enPassant(Piece ***field, Point start_point, Point end_point) const;
...
public:
  ...
  //
  PiecePawn(Point &position, Game *board, Player *player);
  // ----------------------------------------------------------------------
  // Destructor
  //
  ~PiecePawn(void);
  ...
  int getTurnDoubleMove() const; // Todo comment
};

#endif // ifndef PIECEPAWN_H_
